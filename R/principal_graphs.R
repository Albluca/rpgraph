...
projectPoints <- function(Results, Data, TaxonList=NULL, UseR = TRUE, method = 'Dist', Dims = NULL){
  ...
  if(UseR){
    ...
    if(method == 'OnEdges'){
      ...
      RelPosVector <- matrix(rep(NA, nrow(Data)*4), nrow = nrow(Data))
          ...
          if(Inside[j]){
            ...
            if(is.na(onEdge[DistsLists[[i]]$PointsIndices[j]])){
              ...
              RelPosVector[DistsLists[[i]]$PointsIndices[j], ] <-
                c(DistsLists[[i]]$PointsProjections[j], DistsLists[[i]]$Nodes, SegLen[length(SegLen)])
            } else {
      ...
      return(list(PointsOnEdgesCoords = PosVector, OnEdge <- onEdge, EdgeLength = SegLen, PointsOnEdgesDist = RelPosVector))
